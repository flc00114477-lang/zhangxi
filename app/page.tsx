...
async function read(file:File){setMsg("正在讀取月結單…");try{const p=await import("pdfjs-dist/legacy/build/pdf.js");p.GlobalWorkerOptions.workerSrc=new URL("pdf.worker.min.js",window.location.href).toString();const pdf=await p.getDocument({data:new Uint8Array(await file.arrayBuffer())}).promise;let l:string[]=[];for(let n=1;n<=pdf.numPages;n++){const c=await (await pdf.getPage(n)).getTextContent();l.push(...rows(c.items))}const a=l.map((x,i)=>parse(x,i,rs)).filter(Boolean) as T[],u=a.filter((x,i)=>a.findIndex(y=>y.date===x.date&&y.raw===x.raw&&y.amount===x.amount)===i);if(!u.length)throw Error();const s:S={id:String(Date.now()),label:new Date().getFullYear()+" 年 本期",at:new Date().toISOString(),tx:u};setHs(o=>[s,...o].slice(0,24));setSel(s.id);setMsg("已讀取 "+u.length+" 筆交易，已自動完成分類。")}catch(e){const name=e instanceof Error?e.name:"";setMsg(name==="PasswordException"?"PDF 有密碼保護，請先移除密碼後再試。":"未能讀取這份 PDF（"+name+"）。請確認已在「檔案」App 下載完成後再選擇。")}}
