...
export default function Home(){const fi=useRef<HTMLInputElement>(null),bi=useRef<HTMLInputElement>(null);const [hs,setHs]=useState<S[]>(demo),[rs,setRs]=useState<R[]>([]),[sel,setSel]=useState(""),[filter,setFilter]=useState("全部"),[msg,setMsg]=useState("將中銀信用卡月結單 PDF 拖入或選擇"),[ok,setOk]=useState(false);
useEffect(()=>{try{const x=JSON.parse(localStorage.getItem("zhangxi-boc-v1")||"");const clean=(x.history||[]).filter((s:S)=>s.id!=="jun"&&s.id!=="may").map((s:S)=>({...s,tx:s.tx.map((t:T)=>t.category==="待確認"?{...t,category:"其他"}:t)}));if(clean.length){setHs(clean);setSel(clean[0].id)}setRs(x.rules||[])}catch{}setOk(true)},[]);useEffect(()=>{if(ok)localStorage.setItem("zhangxi-boc-v1",JSON.stringify({history:hs,rules:rs}))},[hs,rs,ok]);
...
async function read(file:File){setMsg("正在讀取月結單…");try{const p=await import("pdfjs-dist/legacy/build/pdf.mjs");p.GlobalWorkerOptions.workerSrc=new URL("pdf.worker.min.mjs",window.location.href).toString();const pdf=await p.getDocument({data:new Uint8Array(await file.arrayBuffer())}).promise;let l:string[]=[];for(let n=1;n<=pdf.numPages;n++){const c=await (await pdf.getPage(n)).getTextContent();l.push(...rows(c.items))}const a=l.map((x,i)=>parse(x,i,rs)).filter(Boolean) as T[],u=a.filter((x,i)=>a.findIndex(y=>y.date===x.date&&y.raw===x.raw&&y.amount===x.amount)===i);if(!u.length)throw Error();const s:S={id:String(Date.now()),label:new Date().getFullYear()+" 年 本期",at:new Date().toISOString(),tx:u};setHs(o=>[s,...o].slice(0,24));setSel(s.id);setMsg("已讀取 "+u.length+" 筆交易，已自動完成分類。")}catch(e){const name=e instanceof Error?e.name:"";setMsg(name==="PasswordException"?"PDF 有密碼保護，請先移除密碼後再試。":"未能讀取這份 PDF（"+name+"）。請確認已在「檔案」App 下載完成後再選擇。")}}
